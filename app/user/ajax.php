...
class ajax extends MOX_CONTROLLER
{
    public function check_username_action()
    {
        if ($this->model('user')->check_username_char($_POST['username'])) {
            H::ajax_json_output(MOX_APP::RSM(null, -1, MOX_APP::lang()->_t('用户名不符合规则')));
        }

        if ($this->model('user')->check_username_sensitive_words($_POST['username']) || $this->model('user')->check_username($_POST['username'])) {
            H::ajax_json_output(MOX_APP::RSM(null, -1, MOX_APP::lang()->_t('用户名已被注册')));
        }
    ...
    {
        if (trim($_POST['phone']) == '') {
            H::ajax_json_output(MOX_APP::RSM(null, -1, MOX_APP::lang()->_t('请输入手机号')));
        } else if ($this->model('user')->check_phone($_POST['phone'])) {
            H::ajax_json_output(MOX_APP::RSM(null, -1, MOX_APP::lang()->_t('手机号已经存在')));
        } else if (!H::valid_mobile($_POST['phone'])) {
            H::ajax_json_output(MOX_APP::RSM(null, -1, MOX_APP::lang()->_t('手机号格式有误')));
        }
        ...
        if (!empty($_POST['user_name']) && $this->model('user')->check_username($_POST['user_name'])) {
            ...
            H::ajax_json_output(MOX_APP::RSM(null, -1, MOX_APP::lang()->_t('用户名包含无效字符')));
        } else if ($this->model('user')->check_username_sensitive_words($_POST['user_name']) OR trim($_POST['user_name']) != $_POST['user_name']) {
            H::ajax_json_output(MOX_APP::RSM(null, -1, MOX_APP::lang()->_t('用户名中包含敏感词或系统保留字')));
        }
        ...
        // 检查验证码
        if (!MOX_APP::captcha()->is_validate($_POST['seccode_verify'])) {
        ...
        if ($uid) {
            ...
            H::ajax_json_output(MOX_APP::RSM(null, -1, '注册失败'));
        }
    }
    ...
    public function login_process_action()
    {
        ...
        if ($_POST['type'] == 'vcode') {
            ...
            if (!$user_info) {
                H::ajax_json_output(MOX_APP::RSM(null, -1, '您的手机验证码有误！'));
            }
            ...
            if (!$user_info) {
                H::ajax_json_output(MOX_APP::RSM(null, -1, '请输入正确的帐号或密码'));
            }
        }
    ...
    public function modify_password_action()
    {
        ...
        if (!$user_id) {
            H::ajax_json_output(MOX_APP::RSM(null, '-1', MOX_APP::lang()->_t('请先登录')));
        }

        if (!$_POST['old_password']) {
            H::ajax_json_output(MOX_APP::RSM(null, '-1', MOX_APP::lang()->_t('请输入当前密码')));
        }

        if ($_POST['password'] != $_POST['re_password']) {
            H::ajax_json_output(MOX_APP::RSM(null, '-1', MOX_APP::lang()->_t('请输入相同的确认密码')));
        }
        ...
        if ($this->model('user')->update_user_password($_POST['old_password'], $_POST['password'], $user_id, $user_info['salt'])) {
            H::ajax_json_output(MOX_APP::RSM(null, 1, MOX_APP::lang()->_t('密码修改成功, 请牢记新密码')));
        } else {
            H::ajax_json_output(MOX_APP::RSM(null, '-1', MOX_APP::lang()->_t('请输入正确的当前密码')));
        }
    }
    ...
    public function saveinfo_action()
    {
        ...
        if (!$user_name) {
            H::ajax_json_output(MOX_APP::RSM(null, '-1', MOX_APP::lang()->_t('请输入昵称')));
        }

        if (strlen($_POST['user_name']) < 4 OR strlen($_POST['user_name']) > 30) {
            H::ajax_json_output(MOX_APP::RSM(null, -1, MOX_APP::lang()->_t('昵称长度不符合规则')));
        }

        $user = $this->model('user')->get_user_info_by_username($user_name);
        if ($user && ($user['id'] != $user_id)) {
            H::ajax_json_output(MOX_APP::RSM(null, -1, MOX_APP::lang()->_t('该昵称已经被占用')));
        }

        $result = $this->model('user')->update_user_fields(array('user_name' => $user_name, 'sex' => $sex, 'intro' => $intro, 'last_login' => time()), $user_id);

        if ($result) {
            MOX_APP::model('points')->send($user_id, 'update_user_info');
            H::ajax_json_output(MOX_APP::RSM(null, 1, MOX_APP::lang()->_t('修改成功！')));
        } else {
            H::ajax_json_output(MOX_APP::RSM(null, '-1', MOX_APP::lang()->_t('抱歉，修改失败！')));
        }
    }
    ...
    public function save_avatar_action()
    {
        ...
        if (!$avatar) {
            H::ajax_json_output(MOX_APP::RSM(null, '-1', MOX_APP::lang()->_t('请上传头像')));
        }
