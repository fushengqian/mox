...
switch ($_POST['step'])
{
    default :
        ...
        // 检测 ROOT_PATH /data/ 是否有写权限
        if (is_really_writable(ROOT_PATH . 'data/'))
        {
    ...
    case 2 :
        if (!defined('IN_SAE'))
        {
            ...
            if (! is_really_writable(MOX_PATH))
            {
                $error_messages[] = load_class('core_lang')->_t('目录: %s 无法写入，请将此目录权限设置为 777', MOX_PATH);
            }
        }
    ...
    case 4 :
        ...
        $config_file = file_get_contents(MOX_PATH . 'config.dist.php');
        ...
        file_put_contents(ROOT_PATH . 'data/config/system.php', $config_file);
        file_put_contents(ROOT_PATH . 'data/install.lock', time());
